...
health_stroke_df = pd.read_csv('   ')
health_stroke_df['bmi'].fillna(health_stroke_df['bmi'].median(), inplace=True)
health_stroke_df['smoking_status'].replace('Unknown', 'smokes', inplace=True)

# Upgrade outlier data with decimal points among age
...
health_stroke_df.to_csv("pre_smoked2.csv", index = False)
